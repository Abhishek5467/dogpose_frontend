...
Image Upload and Pose Prediction:
...
Postprocessing & Pose Classification:
...
Aspect ratio < 1.0 → "lying down"

Otherwise, "running"

The classification result is returned as JSON.

Project Setup
Backend Setup
Dependencies:

Python 3.x, Flask, Flask-CORS, TensorFlow (for TFLite), Pillow (PIL), NumPy.
...
In your React Native project, ensure your backend URL is set correctly (e.g., https://dogpose-detector.onrender.com).
...
Follow your normal React Native development process (using Expo or CLI).

ESP32 Setup (Arduino Code)
Arduino IDE Setup:

Install required libraries: WiFi, HTTPClient, and a DHT sensor library.

Configure your WiFi credentials.

Set the server endpoint URL to the Flask backend’s /sensor_data endpoint (preferably over HTTPS).
...
void loop() {
  ...
  // Send data every SEND_INTERVAL if connected
